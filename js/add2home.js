...
var nav = navigator,
	...
	options = {
		...
		message: 'Install canvasClock on your %device: tap %icon and then <strong>Add to Home Screen</strong>.',				// Customize your message or force a language ('' = automatic)
...
if (hasHomescreen && !expired && !isStandalone && isSafari) {
	...
	window.addEventListener("orientationchange", function(){
		var el = document.getElementById('addToHomeScreen');

		startY = isIPad ? window.scrollY : document.documentElement.clientHeight + window.scrollY;
		startX = isIPad ? window.scrollX : Math.round((document.documentElement.clientWidth - el.offsetWidth)/2) + window.scrollX-40;
...
function loaded () {
	...
	setTimeout(function () {
		...
		startY = isIPad ? window.scrollY : document.documentElement.clientHeight + window.scrollY;
		startX = isIPad ? window.scrollX : Math.round((document.documentElement.clientWidth - el.offsetWidth)/2) + window.scrollX;
...
function setPosition () {
	var matrix = new WebKitCSSMatrix(window.getComputedStyle(el, null).webkitTransform),
		posY = isIPad ? window.scrollY - startY : window.scrollY + document.documentElement.clientHeight - startY,
		posX = isIPad ? window.scrollX - startX : window.scrollX + Math.round((document.documentElement.clientWidth - el.offsetWidth)/2) - startX;
...
function addToHomeClose () {
	...
	var posY = isIPad ? window.scrollY - startY : window.scrollY + document.documentElement.clientHeight - startY,
		posX = isIPad ? window.scrollX - startX : window.scrollX + Math.round((document.documentElement.clientWidth - el.offsetWidth)/2) - startX,
		opacity = '1',
		duration = '0',
		close = el.querySelector('.close');
